...
| `KeyboardContracts` | `Direction` enum (NONE + 8 compass), `KeyboardMode` (NORMAL/SHIFTED/CAPS_LOCKED), `LayoutType` (LOGICAL/EFFICIENCY/CUSTOM), `InputMode` (INSTANT=Quick Type/CONFIRM=Steady Type/ASSISTED=One-Handed), `InputAction` enum, `KeyboardActionDelegate` interface |
...
| **3 input modes** | Quick Type (fire on first release), Steady Type (fire when both dials return), One-Handed (lock left dial for one-handed use) |
