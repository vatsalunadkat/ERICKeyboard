...
Status: In progress on `ERICK-141`. `JoystickDrawingUtils.kt` has already been extracted, which reduced `JoystickView.kt` from 1012 lines to 853 lines while keeping `assembleDebug` green. Remaining work is to peel off the 8-section and 6-section drawing flow into narrower units.

---

## Evidence

- `android/app/src/main/java/com/vatoo/erick/JoystickView.kt` started at 1012 lines and is now 853 lines after helper extraction.
...
1. `JoystickDrawingUtils.kt`
2. `JoystickSectionRenderer.kt` for section/ring arc drawing
3. `JoystickCharacterRenderer.kt` for 8-section and 6-section character placement
