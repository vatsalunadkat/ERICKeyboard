...
- Latest finding summary: the current baseline is still split, but the 6-section script now has a real shipped-path configuration instead of only a legacy-path configuration. In addition to the reproduced legacy baseline (`0.94132`, `29.9%`, `72.4` WPM), `erick_v5_6section.py` now supports `ERICK6_UTILITY_MODEL=shipped` and `ERICK6_CORPUS_PROFILE=mixed_shortform`, and a smoke run verified non-zero utility coverage for `TOGGLE_SYMBOLS`, `SPACE`, and `.` with the shipped rotated wheel.
- Evidence reviewed: `docs/documentation/Research/README.md`, `docs/documentation/Research/vatsal/erick_v5_vectorized.py`, `docs/documentation/Research/vatsal/v5_output.txt`, `docs/documentation/Research/vatsal/erick_v5_6section.py`, `docs/documentation/Research/vatsal/results_and_logs/optimization_results_6section_baseline_2026-04-26.md`, `docs/documentation/Research/vatsal/results_and_logs/optimization_results_6section_shipped_mixed_shortform_smoke_2026-04-26.md`, `android/shared/src/commonMain/kotlin/KeyboardLogic.kt`, `docs/documentation/Jira/ERICK-139.md`, and `docs/documentation/Jira/ERICK-148.md`.
- Open blocker: the shipped-path smoke run proves the utility model and benchmark-driven corpus path execute, but Branch 0 still lacks a full comparable shipped-profile optimization run and still approximates symbol-heavy text through `TOGGLE_SYMBOLS` utility tokens instead of a re-optimized symbol layer.
- Next action: run a full shipped-wheel 6-section optimizer pass with the new mixed-text profile, capture a durable result log, and then decide whether Branch 0 can finally freeze a shipped-adjacent baseline or still needs deeper symbol-layer modeling.
